...
./configure --disable-all --enable-static --enable-avformat --enable-avutil --enable-swscale --enable-swresample --enable-vulkan --enable-libdrm --enable-avcodec --enable-decoder=h264 --enable-decoder=hevc --enable-hwaccel=h264_vaapi --enable-hwaccel=hevc_vaapi --enable-hwaccel=h264_vulkan --enable-hwaccel=hevc_vulkan --prefix=/usr "$@" || exit 1
make -j4 || exit 1
make install || exit 1
